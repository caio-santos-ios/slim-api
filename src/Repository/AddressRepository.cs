    ...
    public class AddressRepository(AppDbContext context) : IAddressRepository
{
    ...
    public async Task<ResponseApi<Address?>> GetByIdAsync(string id)
    {
        try
        {
            Address? billing = await context.Addresses.Find(x => x.Id == id && !x.Deleted).FirstOrDefaultAsync();
            return new(billing);
        }
        catch
        {
    ...
    #region CREATE
    public async Task<ResponseApi<Address?>> CreateAsync(Address billing)
    {
        try
        {
            await context.Addresses.InsertOneAsync(billing);
    ...
    #region UPDATE
    public async Task<ResponseApi<Address?>> UpdateAsync(Address billing)
    {
        try
        {
            await context.Addresses.ReplaceOneAsync(x => x.Id == billing.Id, billing);
    ...
    public async Task<ResponseApi<Address>> DeleteAsync(string id)
    {
        try
        {
            ...
            await context.Addresses.ReplaceOneAsync(x => x.Id == id, billing);
